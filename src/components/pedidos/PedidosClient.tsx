...
const b = (v?: 'gold' | 'red' | 'green'): React.CSSProperties => ({ padding: '4px 9px', borderRadius: 6, border: `1px solid ${v === 'gold' ? 'var(--gold)' : v === 'red' ? 'rgba(217,95,95,.25)' : v === 'green' ? 'rgba(76,175,125,.25)' : 'var(--border)'}`, background: v === 'gold' ? 'var(--gold)' : v === 'red' ? 'rgba(217,95,95,.12)' : v === 'green' ? 'rgba(76,175,125,.12)' : 'var(--card)', color: v === 'gold' ? '#0f0f0f' : v === 'red' ? '#d95f5f' : v === 'green' ? '#4caf7d' : 'var(--text)', cursor: 'pointer', fontSize: 11, fontFamily: 'Georgia,serif' })
...
export function PedidosClient() {
  ...
  const [fCanal, setFCanal] = useState('Mostrador')
  const [fPago, setFPago] = useState('Efectivo')
  const [fNotas, setFNotas] = useState('')
  const [fItems, setFItems] = useState<CartItem[]>([])
  const [fProdSel, setFProdSel] = useState('')
  const [fKg, setFKg] = useState(0.5)
  ...
  function addFItem() {
    ...
    setFItems(prev => {
      ...
      return [...prev, { id, nombre: p.nombre, pv: p.precio_venta, qty: fKg }]
    })
  }
  ...
  async function guardarPedido() {
    ...
    setSaving(true)
    try {
      // FIX: usar count separado para evitar bug con tabla vacía
      ...
      if (error) { console.error('Error inserting pedido:', error); alert('Error al guardar: ' + error.message); return }
      await supabase.from('pedido_items').insert(
        fItems.map(i => ({ pedido_id: pd.id, producto_id: i.id, producto_nombre: i.nombre, cantidad_kg: i.qty, precio_unit: i.pv }))
      )
      await supabase.from('comandas').insert({
        numero: 'CP' + num, pedido_id: pd.id, tipo: 'venta',
        contenido: { cliente: fCli, items: fItems, total: totalFItems }, impresa: false,
      })
      setModal(null)
      setFCli(''); setFTel(''); setFItems([]); setFNotas('')
      load()
    } catch (e) { console.error(e); alert('Error inesperado al guardar el pedido') }
    setSaving(false)
  }
  ...
  async function marcarCobrado(id: number) {
    ...
    load()
    setDetalle(prev => prev ? { ...prev, cobrado: true, estado: 'cobrado' } : prev)
  }
  ...
  function impTicket(p: PedidoConItems) {
    ...
    setPrintHTML(`<div style="font-family:'Courier New',monospace;font-size:12px;color:#111;background:#fff;padding:14px;max-width:290px;border:1px solid #ccc">
      <div style="text-align:center;font-weight:bold;border-bottom:1px dashed #000;padding-bottom:5px;margin-bottom:7px">LA ESQUINA DE MADERNA</div>
      <div style="font-size:11px;margin-bottom:7px"><div>Pedido: <strong>${p.numero}</strong></div><div>${p.fecha} · ${p.hora}</div><div>Cliente: ${p.cliente}</div><div>Pago: ${p.medio_pago || '—'}</div></div>
      <div style="border-top:1px dashed #000;margin:5px 0"></div>${rows}<div style="border-top:1px dashed #000;margin:5px 0"></div>
      <div style="font-weight:bold;display:flex;justify-content:space-between;font-size:14px"><span>TOTAL</span><span>${fmt(p.total || 0)}</span></div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10, marginBottom: 16 }}>
        {[['Total pedidos', String(pedidos.length), ''], ['Activos', String(pendientes), '#d95f5f'], ['Cobrados', String(cobrados), '#4caf7d'], ['Facturado hoy', fmt(hoyTotal), '']].map(([l, v, c]) => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <button onClick={() => { setFCli(''); setFTel(''); setFItems([]); setFNotas(''); setModal('nuevo') }} style={{ ...b('gold'), padding: '6px 14px', fontSize: 12 }}>+ Nuevo pedido</button>
      </div>

      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: 14 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr>{['#', 'Fecha', 'Cliente', 'Canal', 'Productos', 'Total', 'Estado', 'Cobrado', ''].map(h => <th key={h} style={{ fontSize: 10, letterSpacing: '1.2px', textTransform: 'uppercase', color: 'var(--muted)', textAlign: 'left', padding: '7px 10px', borderBottom: '1px solid var(--border)' }}>{h}</th>)}</tr></thead>
          <tbody>
            {pedidos.length === 0
              ? <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--dim)', padding: 20 }}>Sin pedidos</td></tr>
              : pedidos.map(p => (
                <tr key={p.id}>
                  ...
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid var(--borderl)', fontSize: 11, color: 'var(--muted)' }}>{(p.pedido_items || []).length} ítems</td>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid var(--borderl)', color: 'var(--gold)' }}>{fmt(p.total || 0)}</td>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid var(--borderl)' }}><span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontSize: 10, color: ESTADO_COLOR[p.estado] || 'var(--muted)', border: `1px solid ${ESTADO_COLOR[p.estado] || 'var(--muted)'}44`, background: `${ESTADO_COLOR[p.estado] || 'var(--muted)'}18` }}>{p.estado}</span></td>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid var(--borderl)' }}>{p.cobrado ? <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, background: 'rgba(76,175,125,.12)', color: '#4caf7d', border: '1px solid rgba(76,175,125,.25)', display: 'inline-block' }}>✓</span> : <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, background: 'rgba(217,95,95,.12)', color: '#d95f5f', border: '1px solid rgba(217,95,95,.25)', display: 'inline-block' }}>Pend.</span>}</td>
                  <td style={{ padding: '8px 10px', borderBottom: '1px solid var(--borderl)' }}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={() => { setDetalle(p); setModal('detalle') }} style={b()}>Ver</button>
                      <button onClick={() => avanzar(p.id, p.estado)} style={b()}>→</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div style={overlay} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={mbox(true)}>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <select value={fProdSel} onChange={e => setFProdSel(e.target.value)} style={{ flex: 2 }}>
                  ...
                  {productos.map(p => <option key={p.id} value={p.id}>{p.nombre} — {fmt(p.precio_venta)}/kg</option>)}
                </select>
                <input type="number" value={fKg} onChange={e => setFKg(parseFloat(e.target.value))} min={0.1} step={0.1} style={{ width: 80 }} />
                <button onClick={addFItem} style={b('green')}>+ Agregar</button>
              </div>
              {fItems.map(i => (
                <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 0', borderBottom: '1px solid var(--borderl)', fontSize: 12 }}>
                  ...
                  <button onClick={() => setFItems(prev => prev.filter(x => x.id !== i.id))} style={{ ...b('red'), padding: '2px 6px' }}>✕</button>
                </div>
              ))}
              <div style={{ textAlign: 'right', marginTop: 8, color: 'var(--gold)', fontSize: 15 }}>Total: {fmt(totalFItems)}</div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button onClick={guardarPedido} disabled={saving} style={{ ...b('gold'), opacity: saving ? 0.6 : 1 }}>{saving ? 'Guardando...' : 'Guardar pedido'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={mbox(true)}>
            ...
            <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)', marginBottom: 16 }}>
              {ESTADOS.map((e, i) => {
                ...
                return <div key={e} onClick={() => avanzar(detalle.id, i > 0 ? ESTADOS[i - 1] : ESTADOS[0])} style={{ flex: 1, padding: '8px 4px', textAlign: 'center', fontSize: 10, letterSpacing: '.3px', textTransform: 'uppercase', color: isActive ? '#0f0f0f' : isDone ? 'var(--gold)' : 'var(--muted)', background: isActive ? 'var(--gold)' : isDone ? 'var(--gold-bg)' : 'var(--surface)', borderRight: '1px solid var(--border)', cursor: 'pointer' }}>{e}</div>
              })}
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 10 }}>
              ...
              <tbody>{(detalle.pedido_items || []).map(i => <tr key={i.id}><td style={{ padding: '7px 8px', borderBottom: '1px solid var(--borderl)' }}>{i.producto_nombre}</td><td style={{ padding: '7px 8px', borderBottom: '1px solid var(--borderl)' }}>{fmtN(i.cantidad_kg, 3)} kg</td><td style={{ padding: '7px 8px', borderBottom: '1px solid var(--borderl)', textAlign: 'right', color: 'var(--gold)' }}>{fmt(i.precio_unit * i.cantidad_kg)}</td></tr>)}</tbody>
            </table>
            <div style={{ textAlign: 'right', fontSize: 17, color: 'var(--gold)', marginBottom: 12 }}>Total: {fmt(detalle.total || 0)}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                {!detalle.cobrado && <button onClick={() => marcarCobrado(detalle.id)} style={b('green')}>✓ Cobrar</button>}
                <button onClick={() => cancelar(detalle.id)} style={b('red')}>Cancelar</button>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => impComanda(detalle)} style={b()}>🗒 Comanda</button>
                <button onClick={() => impTicket(detalle)} style={b()}>🧾 Ticket</button>
                <button onClick={() => setModal(null)} style={b()}>Cerrar</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={overlay} onClick={e => { if (e.target === e.currentTarget) setModal(null) }}>
          <div style={{ ...mbox(), maxWidth: 380 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14 }}>
              <button onClick={print} style={b()}>🖨 Imprimir</button>
              <button onClick={() => setModal(null)} style={b()}>Cerrar</button>
            </div>
          </div>
        </div>
